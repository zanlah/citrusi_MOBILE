...
const RouteDetails = () => {
  const { id, inProximity } = useLocalSearchParams<{
    id: string;
    inProximity?: string;
  }>();
      ...
      <View className="bg-gray-200  min-h-full pt-5 p-2">
        {route && (
          <>
            ...
            <Text className="text-lg mt-1 text-gray-600">
              Opis: {route.abstractDescription}
            </Text>

            <View className="mt-4">
              <Text className="text-lg">
                Čas:{" "}
                <Text className="font-bold">{formatTime(route.duration)}</Text>
              </Text>
              <Text className="text-lg">
                Dolžina: <Text className="font-bold">{route.distance}</Text>
              </Text>
              <Text className="text-lg">
                Višinska razlika:{" "}
              ...
              <Text className="text-lg">
                Plezalna oprema:{" "}
                <Text className="font-bold">
                  {route.hasSafetyGear || "ni potrebna"}
                </Text>
              </Text>
              <Text className="text-lg">
                Koča zaprta:{" "}
                <Text className="font-bold">{route.hutClosed || "Ne"}</Text>
              </Text>
            </View>
          </>
        )}
        {inProximityBool && (
          <Pressable
            className="px-4 py-4 mt-5 justify-center items-center bg-green-600 text-white  rounded-md"
            onPress={startRoute}
          >
